...
//error_log("POST recebido: " . file_get_contents('php://input'));
...
if (preg_match("/^\d{4}-\d{2}-\d{2}$/", $start_date) && preg_match("/^\d{4}-\d{2}-\d{2}$/", $end_date)) {
    ...
    if (!empty($start_date) && !empty($end_date)) {
        ...
        $stmtLote = $mysqli->prepare($sqlLote);
          ...
          if (!$stmtLote) {
             echo json_encode(['status' => 'erro', 'mensagem' => 'Erro ao preparar consulta de lote.', 'erro_sql' => $mysqli->error]);
            exit();
        }
        ...
        //error_log("SQL Final: $sqlLote");
        //error_log("Tipos: $types");
        //error_log("Parâmetros: " . json_encode($paramsLote));
